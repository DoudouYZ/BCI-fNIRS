...
if __name__ == '__main__':
    ...
    # New paired t-tests comparing inverted and same_side vs "all" and vs each other.
    print("\n=== Paired t-tests Comparing Configurations Across Subjects ===")
    for measure in ["combined", "hbo", "hbr"]:
        t_inv_vs_all, p_inv_vs_all = ttest_rel(overall_aggregated["inverted"][measure],
                                               overall_aggregated["all"][measure])
        t_same_vs_all, p_same_vs_all = ttest_rel(overall_aggregated["same_side"][measure],
                                                 overall_aggregated["all"][measure])
        t_inv_vs_same, p_inv_vs_same = ttest_rel(overall_aggregated["inverted"][measure],
                                                 overall_aggregated["same_side"][measure])
        print(f"\nMeasure: {measure}")
        print(f"Inverted vs. All:       t = {t_inv_vs_all:.4g}, p = {p_inv_vs_all:.4g}")
        print(f"Same-side vs. All:      t = {t_same_vs_all:.4g}, p = {p_same_vs_all:.4g}")
        print(f"Inverted vs. Same-side: t = {t_inv_vs_same:.4g}, p = {p_inv_vs_same:.4g}")